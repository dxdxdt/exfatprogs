 ...
 * we're stuck with the 16-bit signed version of FTS interface. The state of FTS
 * and ntfw() in Linux are a mess at the moment. This needs to be addressed -
 * like we've gone through the whole `_FILE_OFFSET_BITS=64` shebang. FTS is still
 * the best option that doesn't involve detrimental recursion.
 */
#include <fts.h>

#include <linux/msdos_fs.h>
#include <linux/exfat.h>
#undef ATTR_HIDDEN
#undef ATTR_VOLUME
...
#ifndef EXFAT_IOC_GET_VALID_DATA
#define EXFAT_IOC_GET_VALID_DATA	_IOR('r', 0x14, __u64)
#endif
#ifndef EXFAT_IOC_SET_VALID_DATA
